...
// POST /admin/mode with {"mode":"pooled"} reports already-pooled; any other
// mode reports that only pooled mode exists. Lane B removes the dashboard
// call sites + dashboard AdminRoutes rows; the integration commit deletes
// this stub together with the server_routes entries.
func (a *adminHandlers) handleModeSwitch(w http.ResponseWriter, r *http.Request) {
	...
	case "pooled", "":
		...
		return
	default:
		a.dash.RenderConfigResult(w, r, false, "Only pooled mode exists (bridge/hybrid removed).")
	}
}

// STUB (pool-only excision, Lane A): per-client bridge token lock/unlock is
// gone with the bridge cache (410 Gone). Lane B removes the dashboard call
// sites + dashboard AdminRoutes rows; the integration commit deletes these
// stubs together with the server_routes entries.
func (a *adminHandlers) handleBridgeTokenLock(w http.ResponseWriter, r *http.Request) {
	a.dash.RenderConfigResult(w, r, false, "Bridge tokens removed (pool-only).")
}

// STUB (pool-only excision, Lane A): see handleBridgeTokenLock.
