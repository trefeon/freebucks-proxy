...
func TestLifecycleFullJourney(t *testing.T) {
	...
	t.Run("config-edit", func(t *testing.T) {
		...
		// Valid edit: LOG_LEVEL change applies and survives. The overlay
		// keeps beating the file, so the added token survives the reload
		// even though the file never learns it (unified-store: no export
		// leg on the token path).
