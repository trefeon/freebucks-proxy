...
func effectiveConfigKV(cfg *config.Config) map[string]string {
	return map[string]string{
		...
		"COOLDOWN_DEFAULT_MS":         strconv.FormatInt(cfg.DefaultMs().Milliseconds(), 10),
		"COOLDOWN_COUNTRY_BLOCK_MS":   strconv.FormatInt(cfg.CountryBlockMs().Milliseconds(), 10),
		"COOLDOWN_CEILING_MS":         strconv.FormatInt(cfg.CeilingMs().Milliseconds(), 10),
		"COOLDOWN_FANOUT_MS":          strconv.FormatInt(cfg.FanoutMs().Milliseconds(), 10),
		"COOLDOWN_INVALID_MODEL_MS":   strconv.FormatInt(cfg.InvalidModelMs().Milliseconds(), 10),
		"COOLDOWN_OPAQUE_MS":          strconv.FormatInt(cfg.OpaqueMs().Milliseconds(), 10),
		"COOLDOWN_LOADSHED_MS":        strconv.FormatInt(cfg.LoadShedMs().Milliseconds(), 10),
		"COOLDOWN_PEAK_HOURS_MS":      strconv.FormatInt(cfg.PeakHoursMs().Milliseconds(), 10),
		"COOLDOWN_IP_MAX_READMITS":    strconv.Itoa(cfg.IpMaxReadmits()),
		"COOLDOWN_IP_JITTER_RATIO":    strconv.FormatFloat(cfg.IpJitterRatio(), 'f', -1, 64),
		"SESSION_PARK_ENABLED":        strconv.FormatBool(cfg.SessionParkEnabled()),
		"SESSION_PARK_THRESHOLD_MS":   strconv.FormatInt(cfg.SessionParkThresholdMs().Milliseconds(), 10),
		"SESSION_POLL_MAX_MS":         strconv.FormatInt(cfg.SessionPollMaxMs().Milliseconds(), 10),
		"SMART_PROBE_BACKOFF_MAX_MS":  strconv.FormatInt(cfg.SmartProbeBackoffMaxMs().Milliseconds(), 10),
		"MATURITY_BACKOFF_MS":         strconv.FormatInt(cfg.MaturityBackoffMs().Milliseconds(), 10),
	}
}
