...
// cooldown holds the slot (true) instead of dropping it: enabled plus
// at-or-below threshold. Unknown/zero cooldowns park (bounded backoff
// below); the fatal set never queries — it stays terminal regardless.
// Exported so the pool's cooldown layer shares the exact gate.
